...
class StatisticsView(viewsets.ViewSet):
    ...
    def revenue_statistics(self, request):
        ...
        # Thống kê theo ngày
        if period == 'day':
            revenue_data = ChiTietOrder.objects.filter(
                ...
                total_revenue=Sum(F('so_luong') * F('gia')),
    ...
    def dish_statistics(self, request):
        ...
        # Query cơ bản
        query = ChiTietOrder.objects.filter(
            order__order_time__date__gte=start_date,
            order__order_time__date__lte=end_date,
            order__trang_thai='completed'
        )
        ...
        if category_id:
            query = query.filter(mon_an__danh_muc_id=category_id)
        
        # Top món bán chạy
        top_dishes = query.values(
            'mon_an__id',
            'mon_an__ten_mon',
            'mon_an__danh_muc__ten_danh_muc'
        ).annotate(
            total_sold=Sum('so_luong'),
            total_revenue=Sum(F('so_luong') * F('gia')),
            total_orders=Count('order', distinct=True)
    ...
    def table_statistics(self, request):
        ...
        # Thống kê sử dụng bàn từ Orders
        table_usage_orders = Order.objects.filter(
            loai_order='dine_in',
            order_time__date__gte=start_date,
            order_time__date__lte=end_date,
            ...
            total_uses=Count('id'),
            completed_uses=Count('id', filter=Q(trang_thai='completed'))
        ).order_by('-total_uses')
        
        # Thống kê đặt bàn từ DonHang
        table_reservations = DonHang.objects.filter(
            ...
            completed_reservations=Count('id', filter=Q(trang_thai='completed'))
        ).order_by('-total_reservations')
        
        # Thống kê theo khu vực
        by_area = Order.objects.filter(
            loai_order='dine_in',
            order_time__date__gte=start_date,
            order_time__date__lte=end_date,
            ban_an__isnull=False
        ).values('ban_an__khu_vuc').annotate(
            total_orders=Count('id')
        ...
        return Response({
            'start_date': start_date.isoformat(),
            'end_date': end_date.isoformat(),
            'table_usage': list(table_usage_orders),
            'table_reservations': list(table_reservations),
    ...
    def customer_statistics(self, request):
        ...
        new_customers = NguoiDung.objects.filter(
            loai_nguoi_dung='khach_hang',
            date_joined__date__gte=start_date,
            date_joined__date__lte=end_date
        ).count()
        
        # Khách hàng thân thiết (nhiều đơn nhất)
        ...
        top_spenders = ChiTietOrder.objects.filter(
            order__order_time__date__gte=start_date,
            order__order_time__date__lte=end_date,
            order__trang_thai='completed',
            order__khach_hang__isnull=False
        ).values(
            'order__khach_hang__id',
            'order__khach_hang__ho_ten',
            'order__khach_hang__so_dien_thoai'
        ).annotate(
            total_spent=Sum(F('so_luong') * F('gia')),
            total_orders=Count('order', distinct=True)
    ...
    def staff_performance(self, request):
        ...
        staff_performance = Order.objects.filter(
            ...
            total_orders=Count('id'),
            completed_orders=Count('id', filter=Q(trang_thai='completed')),
            canceled_orders=Count('id', filter=Q(trang_thai='canceled')),
            pending_orders=Count('id', filter=Q(trang_thai='pending'))
